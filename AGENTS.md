...
Define an operation's responsibility and observable behavior through representative consumer examples. Check how a plausible future requirement could fit without breaking those calls; do not implement speculative extensions. Within a supported release line, preserve documented results, failure handling, retries, and lifecycle guarantees. Use consumer examples as compatibility tests during internal refactors. Widespread use alone is not a reason to reject a useful convenience.
...
Design contracts around coherent capabilities and meaningful substitution. Separate optional capabilities when they need independent implementations, and bind supported contracts coherently so application replacements work. Keep backend-specific behavior off general contracts and internal helpers off the supported API; a convenience does not need an interface solely because it is public.

Within a supported release line, preserve compatibility for consumer implementations and subclasses. Required methods, constructor changes, and new base-class methods can break consuming code; evaluate that impact before changing them. Generated application classes should declare application concerns while Foundation owns its infrastructure.
...
Choose concise names that communicate an object's role in context. Use established terminology and let namespaces supply context; add qualifiers only to resolve actual ambiguity. Constructor arguments and properties should use short, clear role names such as `Compiler $compiler`, `InsertWriter $writer`, and `IdentifierQuoter $quoter`, without mechanically repeating the full class name. Use singular names for individual objects and plural names for collections. Judge names in constructors and ordinary calls, not only in isolation; avoid obscure abbreviations.
...
Give independently invoked CLI operations separate `<Operation>Command` classes and colon-separated names such as `migrate:run`, `migrate:rollback`, and `migrate:status`. Each command declares only its own arguments and options, owns its validation and confirmations, and delegates application work to injected services. Avoid selecting unrelated operations through flags or positional dispatch inside one command. Reuse the existing command base for the relevant CLI; share focused collaborators such as output formatting when multiple commands need them, without adding a base class solely for common constructors or metadata. Update help, documentation, generator instructions, and real CLI tests together when command names or options change.
...
`stellarwp/foundation-database` provides a configured Doctrine DBAL connection, convenient application tables, and reliable WordPress transaction behavior. `stellarwp/foundation-migrations` owns migration discovery, schema declarations and planning, history, failure handling, and migration commands. Developers use Doctrine directly for database expressions and advanced operations. Foundation owns the bounded fluent query API and its SQL compilation. Doctrine owns connection execution, parameter binding, native result handling, schema inspection, and platform schema SQL generation. Foundation owns provider defaults, WordPress connection integration, resource naming, and operational guarantees. Foundation 2.0 targets Doctrine DBAL `^4.4` on PHP 8.3, with its MySQL 5.7.9+ and MariaDB 10.4.3+ server minimums. Keep these requirements visible in the Database and Migrations guides. DBAL 3 support is deferred until the Rector downgrade feature is implemented; it is not a Foundation 2.0 release requirement. Keep dependency-major differences confined to internal connection and schema boundaries, and validate dependency behavior separately from Rector syntax conversion before widening constraints. Future DBAL major lines require a deliberate compatibility decision.

Use Foundation's fluent query builder for ordinary application queries, including escaped substring searches and multi-row writes. Keep its initial surface bounded to demonstrated consumer requirements; Laravel-inspired names do not imply full Laravel parity. Use Doctrine's native `Connection`, query builder, results, exceptions, and schema objects for advanced operations and escape hatches. Do not introduce execution facades or gateway contracts that merely forward native APIs. Keep contracts for meaningful Foundation-owned policies and supported variations. Exposed Doctrine types and behavior are intentional compatibility commitments; do not imply that replacing Doctrine will be transparent to consumers. Preserve Foundation-specific exceptions for scope, migration, and managed-transaction failures.

Application table classes extend `StellarWP\Foundation\Database\Table\Table`. Inject the concrete table for ordinary reads and writes, and the shared Doctrine `Connection` for transactions or cross-table work. `query()` creates a fresh Foundation `Database\Query\Query` with the table configured; do not share a mutable builder between operations. Add conveniences for demonstrated common usage with clear return and failure semantics. Repositories organize application queries when useful; they are not required for every table.

Every table implementation returns a stable unprefixed name from `unprefixedName()`. Resolve and validate physical names through `TableNameResolver` at the point of use, applying the current `DatabaseScope`; application tables and repositories must not repeat WordPress prefix handling or physical-name validation. Application query entry points ordinarily use table objects. The injected `Database\Query\Database` also accepts unprefixed application names and resolved `Query\ValueObjects\TableReference` values, and resolves WordPress core table references through `wordpress()`. Migration schema entry points accept historical unprefixed strings, resolved and validated by the same naming policy; table objects remain accepted for deliberate programmatic use. Native Doctrine and raw SQL use their resolved, quoted names. Generated table classes inherit the base class's infrastructure constructor and declare only application concerns. Keep historical schema definitions in migrations, independent of runtime table classes.
...
Use Doctrine schema objects and platform tools for schema declarations, inspection, comparison, and DDL generation. Plan the current migration against affected live tables, preserving unrelated schema. Do not replay applied migration declarations or reconcile historical schema drift. Preview carries simulated schema through pending operations. Keep the public blueprint API, explicit foreign keys and renames, and supported decimal/timestamp semantics. On failure, stop and leave the migration unrecorded; DDL may have committed, so developers must inspect and repair before retrying. Do not add automatic partial-migration recovery without a demonstrated need and a bounded design.

Migration IDs determine forward execution order and must remain stable across PHP namespace changes and Strauss scoping. Collect all provider contributions, sort globally in ascending byte-exact ID order, and run pending migrations under migration locking. Contribution order must not affect execution. Missing applied declarations remain visible in status but do not block forward work. Preflight only declarations selected for reversal before executing any reversal; refresh selects all applied migrations. Keep recommending migration history for fresh installations and rollback, without requiring old files for unrelated forward work. Use target-version rollback in descending byte-exact ID order. Rollback operations only reverse applied migrations; only migration reconciliation may also apply pending IDs through a target. Developers own migration dependencies and selecting a safe rollback target; document that behavior rather than adding execution-order tracking or batch semantics solely to handle migrations introduced out of order. Generated timestamps provide sortable IDs; custom IDs remain valid and assume responsibility for their lexical position. Foundation owns migration registration, stable identities, WordPress integration, locking, and its documented failure guarantees so application migrations declare their changes without assembling that infrastructure. Foundation owns migration planning, execution, and history. Doctrine supplies schema comparison and platform SQL. Keep DBAL-specific schema translation internal so a future DBAL 3 downgrade does not change application migration declarations.
