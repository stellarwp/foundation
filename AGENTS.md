...
Providers that read application configuration must extend `ConfiguredProvider`; providers that need only the container should extend the narrower `Provider`. Bind the Foundation `Configuration` contract to an application-lifetime configuration snapshot before registering configured providers. Keep third-party configuration implementations such as `Adbar\Dot` behind a Foundation-owned adapter rather than exposing them in provider signatures or application services.
