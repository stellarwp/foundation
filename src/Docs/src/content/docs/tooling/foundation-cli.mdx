...
The command creates `Reports_Table.php` and a timestamped `db/migrations/<timestamp>_create_reports_table.php`. Add the columns to the migration, preview its SQL, and apply it through WP-CLI. Follow the [complete create-and-alter workflow](/components/database/migrations/) to configure discovery and run the generated code.

`--table-name` supplies the stable name before WordPress adds its site prefix. `--migration` creates an initial migration whose rollback drops the complete table. Table `--namespace` and `--path` options affect only the table class. Migrations use `database.migrations.path` from root `config.php`, defaulting to `db/migrations`.
...
Migration placement is shared with runtime discovery through `database.migrations.path`. Configure the [migration directory](/components/database/migrations/#use-another-directory) once. Migration commands have no separate `--path`, `--namespace`, or `--provider` option. Use a name such as `reports/add_status` to generate inside a feature subfolder.
