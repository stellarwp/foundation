...
The `--migration` flag creates and registers the table's initial migration in the same operation. Table `--namespace` and `--path` options affect only the table class; the migration uses its conventional namespace and path. Use `--migration-id=<id>` only when the generated timestamp identifier must be replaced.
