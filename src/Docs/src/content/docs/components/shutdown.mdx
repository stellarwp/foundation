...
final class Provider extends Service_Provider {
	...
	private function register_shutdown_task(): void {
...
The shutdown provider registers the shared task collection, the decorated runner, and one callback on WordPress's `shutdown` action at `PHP_INT_MAX`. Registration alone does not construct or run contributed tasks.
