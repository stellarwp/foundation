...
namespace Plugin\Product_Cache;
...
Register the buffer as a singleton so application services and the shutdown writer receive the same instance. Data collected during the request is then available when the writer is resolved at shutdown. The contextual bindings supply the writer's `$cache_key` and `$ttl` constructor arguments.
...
No application hook is required after `ShutdownProvider` is registered. At WordPress shutdown, Foundation first skips requests where `WP_UNINSTALL_PLUGIN` is defined or `wp_installing()` is true. For eligible requests, it:
