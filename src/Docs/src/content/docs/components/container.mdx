...
Register each provider once per container, in dependency order, before resolving application services. Repeated registration can duplicate contributions or hooks; it is not a way to reset or reconfigure the container.
