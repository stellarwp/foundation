...
Use `column()` when the named helpers do not cover the required MySQL type. Import `StellarWP\Foundation\Database\Table\Column` with the table class's other imports, then add the custom columns inside `definition()`:
