...
```php title="src/Database/Migrations/Create_Reports_Table.php"
...
```php title="src/Database/Provider.php"
<?php declare(strict_types=1);
...
`down()` is application code. Reversing an initial migration drops its complete table, including rows and additions made later. Review the inverse and take the application's required backup before rolling back. `--refresh` prompts for confirmation because it reverses and recreates everything; use `--yes` only when that data loss is intended.
:::
