...
If an application implements `StellarWP\Foundation\Database\Contracts\Table` directly, `unprefixedName()` must return a stable name such as `reports`, never `wp_reports`. The table can then be passed to focused database services such as `TableWriter`, `SchemaInspector`, or `TableNameResolver`. Build table-scoped reads with `QueryBuilder` and a `QueryGateway`. Extend the supplied `Table` base class when the table object itself should expose the query and write convenience methods. Implement `ManagedTable` before passing the table to `Schema::createOrUpdate()` so Foundation can read the complete schema it owns.
:::
