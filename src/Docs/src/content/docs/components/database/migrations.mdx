...
The conventional generator paths and class names require no extra options. When a project uses a different structure, the table generator's `--namespace` and `--path` options customize the table class; its migration uses the project-configured migration namespace and path, falling back to `Database\\Migrations`. Configure persistent defaults for providers, tables, and migrations in [generator locations](/tooling/foundation-cli/#configure-generator-locations). Pass an explicit identifier such as `--migration-id=2026_09_04_143200_create_reports_table` only when the generated timestamp identifier must be replaced.
...
Most application tables should extend the supplied `Table` base class. Implement `StellarWP\Foundation\Database\Contracts\Table` directly only for a custom wrapper that does not need Foundation's query and write conveniences. In either case, `unprefixedName()` must return the stable application table name, such as `your_plugin_reports`, without the WordPress site prefix.
:::
...
| `timestamp( 'occurred_at' )` | `TIMESTAMP`, optionally with precision from `1` to `6` | Times stored with database session time-zone conversion |
...
Available modifiers are `unsigned()`, `nullable()`, `notNull()`, `default()`, `useCurrent()`, `useCurrentOnUpdate()`, `autoIncrement()`, and `comment()`. An explicit `default( null )` is valid only on a nullable column.
...
Use `useCurrent()` for a current-time default and `useCurrentOnUpdate()` for automatic updates when another stored value in the row changes. Both work with `dateTime()` and `timestamp()`. Neither modifier changes nullability, and automatic updates do not imply a current-time default.

In `src/Database/Migrations/Create_Reports_Table.php`, define creation and modification times in the migration's blueprint:
...
An insert that omits these values receives the database's current time. A later update changes `updated_at` but leaves `created_at` alone. An update that leaves every stored value unchanged does not advance the timestamp. Explicit timestamp assignments take precedence; explicitly assigning null stores null on a nullable column.

The column controls fractional-second precision. Omitted precision and `0` mean whole seconds; `1` through `6` retain that many fractional digits. The generated default and update expressions use the same precision. In `src/Database/Migrations/Create_Reports_Table.php`, replace the earlier `updated_at` declaration and optionally add a nullable processing time:

```php title="Create_Reports_Table.php"
$blueprint->dateTime( 'updated_at', 6 )->useCurrent()->useCurrentOnUpdate();
$blueprint->timestamp( 'processed_at', 3 )->nullable()->default( null )->useCurrentOnUpdate();
```

The first declaration emits `DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`. The second stays null on insertion and receives the current time when another stored value changes.

`useCurrent()` and `default()` replace the same default: the last call wins. A string such as `default( 'CURRENT_TIMESTAMP' )` remains literal text, not executable SQL. Foundation rejects the temporal modifiers on incompatible types or invalid precision before executing schema SQL.

:::caution[Database time is not WordPress time]
These values use the database session's clock and time zone, not PHP time or the WordPress timezone setting. `TIMESTAMP` converts between the session time zone and UTC storage; `DATETIME` does not perform that conversion. Keep existing column types unless a migration deliberately changes their meaning.
:::

To change automatic behavior, create a new alteration migration and declare the complete column with `change()`. In `src/Database/Migrations/Stop_Automatic_Report_Updates.php`, for example:

```php title="Stop_Automatic_Report_Updates.php"
public function up( Schema $schema ): void {
	$blueprint = Blueprint::for( $this->table );
	$blueprint->dateTime( 'updated_at' )->useCurrent()->change();

	$schema->alter( $blueprint );
}
```

This keeps the insertion default but removes automatic updates. Include any existing nullability, precision, and comments that must survive the change. To remove current-time initialization too, omit `useCurrent()` and declare the intended literal or nullable null default. Matching retries verify the attributes without replaying DDL or rewriting stored timestamps.

:::caution[Legacy implicit TIMESTAMP defaults]
With `explicit_defaults_for_timestamp=OFF`, the database can add automatic initialization and updates to the first bare `TIMESTAMP` column. Foundation rejects those undeclared attributes rather than changing the connection setting or accepting a different schema. Enable explicit defaults in the database configuration, or declare an explicit default or a nullable column, such as `timestamp( 'processed_at' )->nullable()->default( null )`, then retry. If partial creation left an incompatible table, repair it through an explicit alteration; creation retries only verify existing tables.
:::
