...
```shell
vendor/bin/foundation make:database-migration Add_Published_At_To_Reports \
	--table=Reports_Table
```
...
```php title="Add_Published_At_To_Reports.php"
public function up( Schema $schema ): void {
	$schema->createOrUpdate( $this->table );
}
...
For example, after replacing a single-column `status` index with the composite `status_created_at` index in `Reports_Table::definition()`, make the removal explicit before reconciling the desired definition:

```php title="Replace_Report_Status_Index.php"
public function up( Schema $schema ): void {
...
/**
 * @throws IrreversibleMigration Until the previous index is restored safely.
 */
public function down( Schema $schema ): void {
	throw IrreversibleMigration::forMigration( self::ID );
}
...
use YourPlugin\Database\Migrations\Add_Published_At_To_Reports;
use YourPlugin\Database\Migrations\Backfill_Report_Status;
use YourPlugin\Database\Migrations\Create_Reports_Table;
use YourPlugin\Database\Migrations\Replace_Report_Status_Index;
use YourPlugin\Database\Tables\Reports_Table;
	...
	private function register_migrations(): void {
		$this->container->mergeArrayVar(
			...
			static fn ( C $c ): array => [
				...
				$c->get( Add_Published_At_To_Reports::class ),
				$c->get( Replace_Report_Status_Index::class ),
