...
The generated `src/Database/Tables/Reports_Table.php` owns its logical name and desired schema. `Database::tableName()` applies the current WordPress table prefix.
...
For parameterized data changes, inject the `Database` contract into the migration and pass values through WordPress placeholders. For example, `src/Database/Migrations/Backfill_Report_Status.php` can update existing rows without interpolating values into SQL:
...
Do not interpolate request, configuration, or stored data directly into SQL.
