...
`toQuery()` captures SQL for the active WordPress database scope. If code calls `switch_to_blog()`, build a new query before executing it so the table name uses the new site's prefix. Calling `get()`, `first()`, or `max()` directly on the table query builder resolves the active scope as part of that operation. For a scalar result, call `value()` on the `Query` returned by `toQuery()`.
:::
