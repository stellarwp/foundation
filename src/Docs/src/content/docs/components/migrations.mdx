...
These are deliberate manual SQL changes, not schema repairs. Do not record an incomplete migration merely to suppress an error. Verify or finish its intended schema and data work first. After correcting history, inspect status and preview the next run before resuming upgrades. Keep an operational record of the repair.
