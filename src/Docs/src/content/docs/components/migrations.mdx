...
`migrate($target)` and `migrate:run --to=<id>` reverse applied IDs above the target in descending order, then apply pending IDs through it in ascending order. `latest` applies all pending migrations. `rollbackTo($target)` and `migrate:rollback --to=<id>` only reverse applied IDs above the target; any pending IDs, including the target itself, remain pending. Rollback counts IDs, not deployment batches. Developers own dependencies and choosing a safe target, especially when a newly enabled package adds an older ID. Missing applied files appear in status but do not block new forward migrations. Before rollback, Foundation checks that every migration selected for reversal is available; unrelated missing files do not block it. Refresh requires every applied declaration because it reverses all migrations. Retain migration files for fresh installations and rollbacks; deleting history is not a supported schema-squashing workflow.

:::danger[Rollback can delete data]
`down()` is application code. Reversing an initial migration drops its complete table, including rows and additions made later. Review the inverse and take the application's required backup before rolling back. `migrate:refresh`, `migrate:run --to=0`, and `migrate:rollback --to=0` prompt for confirmation because they reverse every applied migration. Use `--yes` only when that data loss is intended. `migrate:run --to=0 --dry-run` previews the reversal without prompting or executing it.
:::
