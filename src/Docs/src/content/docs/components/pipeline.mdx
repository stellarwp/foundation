...
Use pipes to transform values, short-circuit execution, or wrap `$next( $value )` to transform its result or handle downstream failures. Inject any service dependencies into the pipe and configure its position through the provider.

For a different execution implementation, implement `StellarWP\Foundation\Pipeline\Contracts\Pipeline` and bind it for the consuming service. The contract exposes `send()`, `then()`, and `thenReturn()`; configuration of the replacement belongs to its provider.
...
Pass the Foundation `Resolver` contract when constructing the concrete pipeline. The nullable constructor, `setContainer()`, and protected `getContainer()` lifecycle no longer exist.
...
The concrete pipeline's execution state and helpers are private. Subclasses that used `carry()`, `prepareDestination()`, `parsePipeString()`, `pipes()`, `handleCarry()`, or `handleException()` must move that behavior into pipes or supply an implementation of the execution contract. Wrap `$next()` inside a pipe to process its result or catch downstream exceptions; use the consumer boundary when handling failures from the entire operation.
