...
if ( isset( $_ENV['FOUNDATION_WPCLI_COMMAND_PREFIX'] ) ) {
	$config['wpcli']['command_prefix'] = $_ENV['FOUNDATION_WPCLI_COMMAND_PREFIX'];
}
...
An exception escaping `runCommand()` is displayed as `Error: <message>` and exits with status `1`. Command classes can let service exceptions propagate to this shared presentation boundary.

Add `--debug` to include the exception class, source location, stack trace, and previous exceptions:

```shell
wp your-plugin catalog:sync staging --debug
```

Explicit `WP_CLI::error()` and `WP_CLI::halt()` calls retain their exit behavior. PHP errors such as `TypeError` remain with the host error handler. Calling `runCommand()` directly still throws its original exceptions.

### Release runtime caches between batches

Long-running commands can call the inherited `clearRuntimeCache()` method after each completed batch. For a command with an injected importer that yields batches, the loop inside `runCommand()` can look like this:

```php
foreach ( $this->importer->batches( 100 ) as $batch ) {
...
This clears WordPress's saved query log and flushes the process's runtime object cache when the cache backend supports [`flush_runtime`](https://developer.wordpress.org/reference/functions/wp_cache_supports/). Shared persistent cache entries, action counts, and registered hooks remain intact. If runtime flushing is unsupported, only the query log is cleared.

Call it after WordPress has loaded and the batch has completed its writes. Release application-held batch and query objects as well; cache cleanup cannot release references your own code still retains. Saved query diagnostics are discarded, so inspect them before cleanup when debugging.
