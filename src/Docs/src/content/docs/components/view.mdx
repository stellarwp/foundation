...
:::caution[Reserved view variable names]
`PhpView` uses `foundationViewPath` and `foundationViewData` internally. Passing either name as a view data key throws `InvalidArgumentException`; choose application-specific variable names instead.
:::
