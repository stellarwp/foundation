...
Application table names are chosen by the developer. When [generating a table](/components/database/migrations/#create-and-change-a-table) for a standalone plugin, use a stable name unique to that plugin, such as `--table-name=your_plugin_reports`. WordPress adds its site prefix to produce `wp_your_plugin_reports`; `foundation.prefix` scopes the migration infrastructure listed above.
...
Leave table names empty to use the scoped defaults. Override values are table names without a WordPress prefix; the active database scope applies the correct site prefix. For example, `your_plugin_migrations` becomes `wp_2_your_plugin_migrations` on site 2 of a typical multisite installation. All resulting prefixed table names must fit MySQL's 64-character identifier limit.
...
### Use database services on multisite
...
:::caution[Do not switch blogs inside a migration]
A migration must not call `switch_to_blog()`, even if it would restore the original blog before returning. Foundation can detect a changed context at its lock and ledger boundaries, but it cannot detect a temporary switch that modifies another site's schema and returns to the original site. Switch blogs only around complete `Migrator` operations as shown above.
:::
...
:::note[Network-wide migrations]
Foundation Database currently manages site-specific migration state. It does not provide a network-wide migration ledger for shared global tables.
:::
...
use Plugin\Database as ApplicationDatabase;
...
<CardGrid>
  ...
  <LinkCard
    title="Query Builder"
    description="Read and write through application table classes, with low-level SQL available when needed."
