...
`foundation.prefix = your-plugin` gives the migration ledger the unprefixed name `your_plugin_foundation_migrations`. WordPress adds its current site prefix. The migration advisory lock is derived from the database and ledger names, so applications with different ledgers migrate independently.
