...
:::caution[Do not leave a migration in another blog context]
A migration may temporarily call `switch_to_blog()` only if it restores the original blog before returning. If the context changes during an operation, Foundation aborts and leaves the original lock to expire instead of risking a write to another site's ledger or lock table.
:::
