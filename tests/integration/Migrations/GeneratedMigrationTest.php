...
final class GeneratedMigrationTest extends TestCase
{
	...
	public function test_generated_migrations_load_without_application_classes_or_migration_autoload_mappings(?string $path): void {
		...
		if ($path !== null) {
			$configuration['migrations']['path'] = $path;
		}

		file_put_contents($root . '/composer.json', json_encode(['autoload' => ['psr-4' => $mappings], 'require' => ['stellarwp/foundation-migrations' => '^2.0']], JSON_THROW_ON_ERROR));
